...
SD = config["vgp_dir"]#"/project/mchaisso_100/cmb-16/mchaisso/projects/VGP"#os.path.dirname(workflow.snakefile)

SeD= config["sedef_dir"]


assembly=config["asm"]
asmm="assembly.orig.fasta"
...
rule MaskFasta:
    ...
    shell:"""
mkdir -p {params.temp}/$$
cp \"{input.part}\" \"{params.temp}/$$/to_mask.{wildcards.region}.fasta\" && \
pushd {params.temp}/$$ &&  \
RepeatMasker {params.repeatLibrary} -pa 8  -s -xsmall \"to_mask.{wildcards.region}.fasta\" && \
popd && \
cp {params.temp}/$$/to_mask.\"{wildcards.region}\".fasta.* split/ || true
...
rule MapIsoSeq:
    ...
    shell:"""
mkdir -p IsoSeq
minimap2 -t 4 -ax splice -uf -C5 {input.assembly} {input.rna} | samtools view -uS - | samtools sort -T {params.temp} -m2G -o {output.aln}
